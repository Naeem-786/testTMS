                ...
                <div class="table-responsive" id="raw_category">                  
                    ...
                    <table id="myTable" class="table  datanew">
                        <thead>
                            ...
                            ?>
                            <h2 class="text-center h2_heading"><?php echo $total_orders?>:Orders Near to Delivery Date</h2>
                            <tr>
                                <th>Sr.No.</th>
                                <th>Order Dated</th>
                                <th>Suit No.</th>
                        ...
                        <tbody>
                            <?php                               
                                $query = "SELECT * FROM addneworder WHERE delivery_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 2 DAY) AND progress_suit IN ('cutting_unit', 'stitching_unit') ORDER BY delivery_date ASC";                                                              
                                $date_expire_record = mysqli_query($conn, $query);            
                                $total_rows = mysqli_num_rows($date_expire_record);

                                if($total_rows >0){
                                    $number = 1;
                                    
                                    while($result = mysqli_fetch_assoc($date_expire_record)){
                                        ...
                                        echo "
                                                <tr>
                                                    ...
                                                    <td>
                                                    <a class='me-3 edit_btn' href='/SM-IMS/edit_invoice.php?id=$result[id]' ><img src='assets/img/icons/dash2.svg' alt='img'></a>
