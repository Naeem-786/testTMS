                    ...
                    <table id="myTable" class="table  datanew">
                        <thead>
                            <?php
                                ...
                                $count_query = "SELECT COUNT(*) AS total_orders FROM addneworder WHERE progress_suit IN ('ready_for_delivery') ORDER BY delivery_date ASC";
                                $count_result = mysqli_query($conn, $count_query);
                                $count_row = mysqli_fetch_assoc($count_result);
                                $total_orders = $count_row['total_orders']; // Get the total count
                            ?>
                            <h2 class="text-center h2_heading"><?php echo $total_orders?>:Order Ready For Delivery</h2>
                            <tr>
                                <th>Sr.No.</th>
                                <th>Order Dated</th>
                                <th>Suit No.</th>
